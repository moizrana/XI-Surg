...
public class ChatWindow : MonoBehaviour
{
    ...
    public Button responseExitButton;
    ...
    private Stack<string> messageHistory = new Stack<string>();
    ...
    void InitializeChatSystem()
    {
        ...
        // Response exit button setup
        if (responseExitButton != null)
        {
            responseExitButton.onClick.AddListener(CloseResponse);
        }
    ...
    public void CloseChat()
    {
        ...
        if (fadeCoroutine != null) StopCoroutine(fadeCoroutine);
        fadeCoroutine = StartCoroutine(Fade(chatCanvasGroup, fadeOutDuration, 0f, () => {
            chatPanel.SetActive(false);
    ...
    {
        // If there's text in the input field, clear it
        if (!string.IsNullOrEmpty(messageInputField.text))
        {
            // Save current text to history before clearing
            messageHistory.Push(messageInputField.text);
            messageInputField.text = string.Empty;
            return;
        }

        // If input field is empty but history has messages, restore last message
        if (messageHistory.Count > 0)
        {
            messageInputField.text = messageHistory.Pop();
            messageInputField.caretPosition = messageInputField.text.Length;
            return;
        }

        // If no text and no history, close the chat
        CloseChat();
    }
    ...
    {
        if (fadeCoroutine != null) StopCoroutine(fadeCoroutine);
        fadeCoroutine = StartCoroutine(Fade(responseCanvasGroup, fadeOutDuration, 0f, () => {
            responsePanel.SetActive(false);
        }));
    }
